...
#include <chrono>
#include <cmath>
// g++ -O3 -std=c++11 gs2D-omp.cpp && ./a.out
// g++ -O3 -fopenmp -std=c++11 gs2D-omp.cpp && ./a.out

#include <iostream>
using namespace std;
const long MAXITER = 501;
void gs2d_seq(double *u, const double *f, long n)
{
  if (n == 0)
    return;
  double h2;
  ...
  for (int iter = 0; iter < MAXITER; ++iter)
  {
    ...
    for (int i = 1; i < n - 1; ++i)
    {
      for (int j = 1; j < n - 1; ++j)
      {
        ...
        u[i * n + j] = 0.25 * (-h2 * f[i * n + j] + u[(i - 1) * n + j] + u[i * n + j - 1] + u[(i + 1) * n + j] + u[i * n + j + 1]);
        // if (i == 2 && j == 3)
        // {
...
void gs2d_omp(double *u, const double *f, long n)
{
  if (n == 0)
    return;
  double h2;
...
int main()
{
  ...
  // double err = 0;
  // for (int i = 0; i < n * n; i++)
  //   // err = std::max(err, std::abs(u1[i] * n * n - u2[i] * n * n));
  //   cout << u1[i] << " " << u2[i] << endl;
  // printf("error = %ld\n", err);
